  ...
  return (
    ...
    <Container>
        <Row>
            <Col>
                <Form>
                    <Form.Control type="number" placeholder="Price" value={price} onChange={handlePrice}/> 
                </Form>
            </Col>
            <Col>
                <Form>
    ...
    <Table stripped="true" bordered hover variant="dark" size="sm">
        ...
        <tr key={listing.ListingId}>
            <td><Link to={`/user/${listing.UserId}`} target="_top">{listing.Username}</Link></td>
            <td><Link to={`/boardGame/${listing.BoardGameId}`} target="_top"> {listing.BoardGameName} </Link></td>
            <td>{listing.Price}</td>
            <td>{listing.Condition}</td>
            <td>{listing.TimeCreated}</td>
            <td><ListingDelete ListingId={listing.ListingId} /></td>
            <td><Link to={`/listing/${listing.ListingId}`} target="_top"><Button class="btn btn-light">Edit</Button></Link></td>
        </tr>
        )}
        </tbody>
    </Table>
